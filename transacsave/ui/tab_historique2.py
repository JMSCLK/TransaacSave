...
class OngletHistorique(BoxLayout):
    ...
    def _ajouter_ligne_tableau(self, tx: dict, index: int):
        ...
        # ── Colonne 3 : Nom (tronqué à 12 caractères) ──
        nom = tx["nom"][:12] + "…" if len(tx["nom"]) > 12 else tx["nom"]
        lbl_nom = Label(
            text=nom, font_size=dp(10), color=COULEUR_TEXTE,
            size_hint=(COLONNES_TABLEAU[2][1], None), height=h,
            halign="left", valign="middle"
        )
        lbl_nom.bind(size=lbl_nom.setter("text_size"))
        self._appliquer_fond(lbl_nom, fond)
        ...
        self.grille_tableau.add_widget(lbl_nom)
